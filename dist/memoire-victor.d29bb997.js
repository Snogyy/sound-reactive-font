...
userInput.addEventListener("input", ()=>{
    const text = userInput.value || "Clap your hands!";
    el.textContent = ""; // vide les anciennes lettres
    strArray = text.split(""); // mets à jour le tableau
    initElements(); // régénère les lettres animées
});
...
function initVisualizer() {
    ...
    navigator.mediaDevices.getUserMedia({
        ...
        function setFontFeatures(avg) {
            ...
            avgHistory.push(avg);
            if (avgHistory.length > els.length) avgHistory.shift();
            for(let i = els.length - 1; i >= 0; i--){
                const el = els[i];
                el.style.fontVariationSettings = `"wght" ${map(avgHistory[i], 0, 64, 10, 100)}`;
            }
        }
        function draw(now) {
...
},{"./styles.css":"lW6qc"}],"lW6qc":[function() {},{}]},["7SvX3","kyksZ"], "kyksZ", "parcelRequire5898", {})

//# sourceMappingURL=memoire-victor.d29bb997.js.map
